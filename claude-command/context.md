...
The first token is the session id prefix; everything after it is the user's focus query (may be empty).
...
3. If a focus query is present, do not stop at the digest: Grep the full transcript at the jsonl path shown in the digest for terms related to the query, and read only the matching parts. Transcript lines are JSON; the conversation text lives in the `message.content` fields.
